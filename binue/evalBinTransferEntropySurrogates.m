function [threshold] = evalBinTransferEntropySurrogates(dataPreprocessed,idTargets,idDrivers,secondIdConditionalTerm,methodParams,numSurrogates,firstEntropyTerm)
...
%     nPoints               = length(dataPreprocessed(1,:));
    ...
    if (methodParams.usePresent == 0)
        for i = 1 : numTargets%parfor
            ...
            for j  = 1 : numDrivers
                driverShiftedMtx{1,j}        = surrotimeshift(dataPreprocessed(currIdDrivers(j,1),:)',tauMin,numSurrogates);
            end
    ...
    %     idDrivers taken once
        ...
        for i = 1 : numTargets
            ...
            driverShiftedMtx             = cell(1,numDrivers);
            for j  = 1 : numDrivers
                driverShiftedMtx{1,j}        = surrotimeshift(dataPreprocessed(currIdDrivers(j,1),:)',tauMin,numSurrogates);%surrotimeshift(dataPreprocessed(reducedDrivers(currIdDrivers(j,1),i),:)',tauMin,numSurrogates);
            end
