function [finalConditionalEntropy minEntropies finalCandidatesMtx condMutualInfo threshold] = evaluateLinearNonUniformEntropy(dataPreprocessed,idTargets,idCandidates,methodParams)
% 
% Syntax:
% 
...
%     chosen term will be deleted from the list
    ...
    shannonEntropyTarget        = shannonLinearEntropy(dataPreprocessed(idTargets(1,k),:));
...
%     term that it is not been chosen during the prevoius steps
    while (~isempty(candidatesWhileCondition(:)))
        ...
        numStackCandidates  = find(candidatesMtx(:,1) > 0);
...
%             evaluating the linear entropy
            [currEnt,~,currEntMtx]   = linearEntropy(dataPreprocessed,idTargets(1,k),{currCandidate});
...
%         evaluating the conditional mutual information
    ...
    %     to the minimum entropy value
        th(1,z)            = evalLinearTestSurrogates(currEntropyMtx{1,idMin},minEntropyValues(1,z),alphaPercentile,numSurrogates);
        %% Step 5
