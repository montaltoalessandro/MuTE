function [conditionalEntropyTerm] = evalNearNeiConditionalEntropy2(dataPreprocessed,idTargets,idToRemove,finalCandidatesMtx,firstEntropyTerm,minEntropies,methodParams,metric,k,funcDir,distance)
% 
% Syntax:
% 
...
% nonUniformTransferEntropy
    ...
    for i = 1 : numTargets
        currIdCandidatesToRemove            = find(idToRemove(:,i) > 0);
        currCandidatesToRemove              = idToRemove(currIdCandidatesToRemove,i);
        ...
        for j = 1 : numCurrFinalCandidates
            ...
            if (~isempty(checkPresenceDriver))
                currFinalCandidates(checkPresenceDriver,:)  = [];
        ...
        idConditionalTerm{1,1}              = currFinalCandidates;
        if (numDriversDeleted > 0 && ~isempty(currFinalCandidates))
            conditionalEntropyTerm(1,i)     = nearNeiConditionalEntropyRiduced(dataPreprocessed',idTargets,idConditionalTerm,metric,k,funcDir,distance);
        elseif (numDriversDeleted > 0 && isempty(currFinalCandidates))
            conditionalEntropyTerm(1,i)     = minEntropies{1,i}(1,1);
        else
