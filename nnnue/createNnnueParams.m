function [paramsNonUniTENearNeighbour] = createNnnueParams(varargin)
% 
% Syntax:
% 
...
paramsNonUniTENearNeighbour                                 = struct;

paramsNonUniTENearNeighbour.idTargets                       = varargin{1,2};
paramsNonUniTENearNeighbour.idDrivers                       = varargin{1,3};
paramsNonUniTENearNeighbour.idOthersLagZero                 = varargin{1,4};
paramsNonUniTENearNeighbour.modelOrder                      = varargin{1,5};
paramsNonUniTENearNeighbour.infoSeries                      = [(1:numSeries)' , ones(numSeries,1).*paramsNonUniTENearNeighbour.modelOrder,ones(numSeries,2)];
paramsNonUniTENearNeighbour.multi_bivAnalysis               = varargin{1,6};
paramsNonUniTENearNeighbour.secondTermCaseVect              = varargin{1,7};
paramsNonUniTENearNeighbour.numSurrogates                   = varargin{1,8};
paramsNonUniTENearNeighbour.metric                          = varargin{1,9};
paramsNonUniTENearNeighbour.numNearNei                      = varargin{1,10};
paramsNonUniTENearNeighbour.informationTransCriterionFun    = varargin{1,11};
paramsNonUniTENearNeighbour.surrogatesTestFun               = varargin{1,12};
paramsNonUniTENearNeighbour.funcDir                         = varargin{1,13};
paramsNonUniTENearNeighbour.homeDir                         = varargin{1,14};
paramsNonUniTENearNeighbour.alphaPercentile                 = varargin{1,15};
% ******** Set the following fields together *******
paramsNonUniTENearNeighbour.genCondTermFun                  = varargin{1,16};
paramsNonUniTENearNeighbour.usePresent                      = varargin{1,17};
% **************************************************
