function [CMI] = evalConditionalMutualInfo(B,metric,k,funcDir,homeDir)
...
%     countA(n)=countA(n)-sum(tmp{n}==dd(n));
    countA(n)=max(k-1,countA(n)-sum(tmp{n}==dd(n)));
end
...
%     countB2(n)=countB2(n)-sum(tmp{n}==dd(n));
    countB2(n)=max(k-1,countB2(n)-sum(tmp{n}==dd(n)));
end
...
%         countA2(n)=countA2(n)-sum(tmp{n}==dd(n));
        countA2(n)=max(k-1,countA2(n)-sum(tmp{n}==dd(n)));
    end
