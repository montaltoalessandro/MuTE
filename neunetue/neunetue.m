function [output] = neunetue(data,methodParams)
...
%     disp('Evaluating Prediction Error');
    for i = 1 : numTargets
        if (strcmp(multi_bivAnalysis,'multiv'))
            if (i == 1)
                [secondNetwork{1,i},secondPredictionTerm(1,i)]     = evalPredErrNNUe(data,methodParams,secondIdCandidates,i);
            elseif (i >= 2)
    ...
    parfor i = 1 : numTargets%parfor
            [firstNetwork{1,i},firstPredictionTerm(1,i)] = evalPredErrNNUe2(data,methodParams,secondNetwork{1,i},idTargets(1,i),idDrivers(:,i),secondIdCandidates{1,i},secondPredictionTerm(1,i));
    end
    
    threshold = evalNeuNetUeEntropySurrogates(data,idTargets,idDrivers,secondIdCandidates,methodParams,numSurrogates,firstPredictionTerm);
